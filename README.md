...
POST /api/parse → Upload lease, parse via Gemini

POST /api/google/events → Create rent + renewal events

POST /api/google/gmail/draft → Generate maintenance email draft

GET /api/ics/* → Download rent/renewal .ics files

👥 Team

Zachary Stybel • Ye Moe • Tamim Kabir
